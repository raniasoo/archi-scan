      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-56 rounded-xl border border-border bg-card shadow-xl z-50 overflow-hidden">
          <div className="p-2 space-y-1">
            {/* 카카오 */}
            <button
              onClick={() => { setOpen(false); signIn("kakao") }}
              className="w-full flex items-center gap-3 rounded-lg px-3 py-2.5 hover:bg-[#FEE500]/20 transition-colors"
            >
              ...
              <span className="text-sm font-medium text-foreground">카카오로 시작</span>
            </button>

            {/* 네이버 */}
            <button
              onClick={() => { setOpen(false); signIn("naver") }}
              className="w-full flex items-center gap-3 rounded-lg px-3 py-2.5 hover:bg-[#03C75A]/10 transition-colors"
            >
              <div className="w-8 h-8 rounded-full bg-[#03C75A] flex items-center justify-center flex-shrink-0">
                <span className="text-white text-xs font-black">N</span>
              </div>
              <span className="text-sm font-medium text-foreground">네이버로 시작</span>
            </button>

            {/* 구글 */}
            <button
              onClick={() => { setOpen(false); signIn("google") }}
              className="w-full flex items-center gap-3 rounded-lg px-3 py-2.5 hover:bg-muted/80 transition-colors"
            >
              ...
              <span className="text-sm font-medium text-foreground">구글로 시작</span>
            </button>
          </div>
        </div>
      )}
