...
export function LayoutSketch({ type, size = 80, className = "" }: LayoutSketchProps) {
  ...
  // 공통 스타일
  const site = { stroke: "currentColor", strokeWidth: 1, strokeDasharray: "3,2", fill: "none", opacity: 0.3 }
  const building = { fill: "hsl(var(--primary))", opacity: 0.6, rx: 1 }
  const buildingStroke = { stroke: "hsl(var(--primary))", strokeWidth: 0.8, fill: "none", opacity: 0.4 }
  const green = { fill: "#22c55e", opacity: 0.15, rx: 2 }
  const road = { fill: "currentColor", opacity: 0.08 }
  const label = { fill: "currentColor", opacity: 0.35, fontSize: s * 0.065, textAnchor: "middle" as const, fontWeight: 600 }

  return (
    <svg width={s} height={s} viewBox={`0 0 ${s} ${s}`} className={className}>
      {/* 도로 (하단) */}
      <rect x={0} y={s - s * 0.1} width={s} height={s * 0.1} {...road} />
      <text x={s / 2} y={s - s * 0.03} {...label} opacity={0.2} fontSize={s * 0.05}>도로</text>
      ...
      {type === "tower" && (
        <>
          ...
          {/* 주차 진입로 */}
          <line x1={s * 0.5} y1={s * 0.6} x2={s * 0.5} y2={s * 0.9} stroke="currentColor" strokeWidth={0.5} opacity={0.15} strokeDasharray="2,2" />
      ...
      {type === "linear" && (
        <>
          ...
          <text x={s * 0.5} y={s * 0.71} {...label} fontSize={s * 0.05} opacity={0.2}>주차</text>
      ...
      <g transform={`translate(${s - s * 0.12}, ${s * 0.06})`}>
        <text fill="currentColor" opacity={0.25} fontSize={s * 0.07} textAnchor="middle" fontWeight={700}>N</text>
        <line x1={0} y1={s * 0.02} x2={0} y2={s * 0.07} stroke="currentColor" strokeWidth={1} opacity={0.2} />
        <polygon points={`0,${s * 0.01} ${-s * 0.02},${s * 0.04} ${s * 0.02},${s * 0.04}`} fill="currentColor" opacity={0.2} />
      </g>
    </svg>
  )
}
