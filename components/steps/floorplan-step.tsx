...
import { ChevronRight, ChevronLeft, Maximize2, Minimize2, CheckCircle2, AlertCircle, ChevronDown } from "lucide-react"
      ...
      <div className="rounded-xl border border-border bg-card overflow-hidden">
        ...
        {isFloorTab && (
          <div className="flex items-center gap-2 px-3 py-2 border-b border-border/50 bg-secondary/10">
            ...
            <button onClick={() => setIsFloorPlanFullscreen(!isFloorPlanFullscreen)} className="p-1 rounded hover:bg-secondary/50">
              {isFloorPlanFullscreen ? <Minimize2 className="h-3.5 w-3.5" /> : <Maximize2 className="h-3.5 w-3.5" />}
            </button>
          </div>
        )}

        {/* 도면 뷰어 */}
        <div className={`p-3 ${isFloorPlanFullscreen && isFloorTab ? 'fixed inset-0 z-50 bg-background p-4 flex flex-col' : ''}`}>
          {isFloorPlanFullscreen && isFloorTab && (
            <div className="flex items-center justify-between mb-3 shrink-0">
              <span className="text-sm font-bold">{selectedFloor}층 {selectedFloor === 1 ? '로비/상가' : selectedFloor === totalFloors ? '최상층' : '기준층'}</span>
              <button onClick={() => setIsFloorPlanFullscreen(false)} className="p-2 rounded-lg hover:bg-secondary/50">
          ...
          {/* 기본 평면 */}
          {drawingTab === "floor" && (
            <div className={isFloorPlanFullscreen ? 'flex-1 flex items-center justify-center' : ''}>
              <div className="w-full" style={isFloorPlanFullscreen ? { maxHeight: 'calc(100vh - 120px)' } : { aspectRatio: '3/2', minHeight: 260, maxHeight: 420 }}>
                <FloorPlan
